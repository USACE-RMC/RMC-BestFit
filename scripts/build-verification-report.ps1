...
$htmlPath = Join-Path $temporaryDirectory "rmc-bestfit-verification-report.html"
$equationTexPath = Join-Path $temporaryDirectory "verification-report-equations.tex"
$equationDviPath = Join-Path $temporaryDirectory "verification-report-equations.dvi"
$equationDirectory = Join-Path $temporaryDirectory "equations"
$rawPdfPath = Join-Path $temporaryDirectory "rmc-bestfit-verification-report.raw.pdf"
$finalPdfPath = Join-Path $outputDirectory "rmc-bestfit-verification-report.pdf"
...
$equationAssets = @(Get-ChildItem -LiteralPath $equationDirectory -Filter "eq-*.svg" -File)
foreach ($asset in $equationAssets) {
...
if ($LASTEXITCODE -ne 0) {
    throw "Chromium failed while printing the verification report PDF."
}
...
if (!(Test-Path -LiteralPath $rawPdfPath -PathType Leaf)) {
    throw "Chromium did not produce the raw verification-report PDF within 15 seconds."
}

& $pythonExecutable (Join-Path $PSScriptRoot "finalize-verification-report-pdf.py") $rawPdfPath $finalPdfPath
if ($LASTEXITCODE -ne 0 -or !(Test-Path -LiteralPath $finalPdfPath -PathType Leaf)) {
    throw "The final verification-report PDF was not produced."
}

Write-Host "Verification report PDF: $finalPdfPath"
exit 0
