...
For a nonstationary univariate model, the candidate vector contains trend coefficients. The parameter priors in Equation (1) apply to those coefficients. Distribution-scale and quantile-prior terms are evaluated using distribution parameters predicted at the last index in `DataFrame.FullTimeSeries`; `ParameterTimeIndex` is reserved for prediction and display. This convention must be considered when placing a quantile prior on a changing distribution, because it anchors the expert statement to the final observed time step.
