...
At the `MCMCSampler` level, `AcceptanceRates[c]` is accepted raw transitions divided by attempted transitions for chain $c$ for every sampler. BestFit report bands are 0.23–0.44 for DE-MC and 0.20–0.30 for ARWMH, with wider warning buffers. Acceptance is an efficiency diagnostic, not a convergence test; an apparently good rate can coexist with mode trapping.

NUTS accepts each completed transition, so its generic sampler counter is normally 1.0. `NUTS.HamiltonianAcceptanceRates[c]` is the separate mean post-warmup Hamiltonian acceptance statistic used for tuning. For NUTS only, construction of `MCMCResults` copies that statistic into the existing `MCMCResults.AcceptanceRates` field. BestFit persists and reports this acceptance value with a target of 0.80 and NUTS-specific wording.

The live `NUTS` sampler also exposes diagnostic transition counts, divergences, maximum-tree-depth hits, mean tree depth, mean leapfrog steps, final step size, and E-BFMI. E-BFMI is computed per chain as $\operatorname{mean}(\Delta E^2)/\operatorname{var}(E)$ from streaming post-warmup energies. These arrays are non-null, require constant memory, and are not copied into `MCMCResults` or displayed by BestFit. Direct Numerics callers may compare E-BFMI with [`rstan::check_energy()`](https://mc-stan.org/rstan/reference/check_hmc_diagnostics.html), which uses 0.2, or CmdStan's more conservative nominal threshold of 0.3 in [`diagnose`](https://mc-stan.org/docs/2_39/cmdstan-guide/diagnose_utility.html). Any divergence, repeated maximum-depth saturation, low E-BFMI, poor R-hat/ESS, or posterior mass against parameter bounds requires investigation.
