...
Profile-interval construction probes finite coordinates outward from the estimate, expands the distance until the threshold is bracketed, and reoptimizes nuisance parameters at each probe. The warm-start cache reuses nearby fitted nuisance coordinates. A finite model boundary can be returned when the profile never crosses the threshold before that boundary; this is a support-limited endpoint. An unavailable probe causes a bounded back-off search, and failure to obtain a finite bracket throws. Infinite parameter bounds are never passed directly to the root solver. Thus a successful interval reports a threshold crossing or a finite support boundary, not an invented finite replacement for infinity.
