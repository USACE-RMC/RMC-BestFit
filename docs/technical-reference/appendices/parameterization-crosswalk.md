...
| `SpatialGEV` | copula correlation block; location regression; scale regression; shape regression; enabled location/scale/shape error blocks (process densities in `PriorLogLikelihood`) | Complete (TR-048 through TR-058, TR-061, TR-062 corrected; TR-059 and TR-060 open) |
