...
The seasonal shift has a useful check: for $0<p_s<1$, $\log p_s<0$, so shortening exposure lowers the Gumbel location. Expanding $p_s^{-\kappa}=1-\kappa\log p_s+O(\kappa^2)$ in the nonzero-shape formula gives the same positive coefficient on $\log p_s$. Exposure changes the annual-maximum distribution even when the conditional exceedance magnitudes are unchanged.
...
A useful threshold is high enough for the tail approximation to be credible but low enough to retain adequate information. **ThresholdDiagnostics**, **MeanResidualLifeResult**, and **ParameterStabilityResult** support threshold sensitivity work. Examine mean residual life, shape/modified-scale stability, event independence, seasonal changes, and decision-relevant quantiles together when judging a threshold [1](#ref-1).
