...
| Finite mixture | Three `MixtureRecoveryTests.*_Recovery_Parity` methods, three corresponding `*_BayesianRecovery` methods, and `InformativePrior_EmSeededMapInitialization_UsesFullPosterior` | BestFit production generator at $n=1000$ and seed 12345; Numerics cross-engine likelihood/EM parity; EM-seeded prior-aware local MAP and posterior population; known synthetic parent recovery through `MixtureAnalysis` | Public EM parity remains passed. Fast initializer contracts pass; the informative-prior method and three Bayesian recovery methods are ready for focused execution after the initializer change. |
| Competing risks | Four passed `CompetingRiskDependencyVerificationTests` methods plus 14 passed/6 failed `CompetingRiskRecoveryTests` methods | Gaussian-copula rank/CDF identities; eight pinned Numerics min/max recovery fixtures; two correlation-matrix extensions; MLE and Default DEMCzs with MAP-centered initialization | TR-012 remains complete. All MLE cells pass; MAP initialization resolves the separated three-Weibull R-hat finding, but six Bayesian findings still keep the recovery supplement and Phase 5 gate open. |
| Composite/model average | Fast `PosteriorIndexResamplerTests`, `CompositePhase4Tests`, `CompositeChildTypeWideningTests`, and UI seed/matrix tests; passed `PosteriorResamplingVerificationTests`; nine passed/one failed `CompositeRecoveryTests` methods | Deterministic criterion/weight identities, structural matrix contracts, R `mistr` Table 45, exact min/max formulas, bivariate-Normal orthants, and complete three-child Cartesian posteriors | TR-013/TR-014/TR-015 remain complete. The extreme-tail inversion finding keeps the recovery supplement open. B17C participates in Equal/AIC/BIC/RMSE and is zero-weighted rather than type-rejected for unavailable posterior criteria. |
