...
| Finite mixture | Three `MixtureRecoveryTests.*_Recovery_Parity` methods and three corresponding `*_BayesianRecovery` methods | BestFit production generator at $n=1000$ and seed 12345; Numerics cross-engine likelihood/EM parity; EM-seeded identified posterior population; known synthetic parent recovery through `MixtureAnalysis` | Public EM parity remains passed. Deterministic contracts cover $K-1$ sampling and local full-$K$ reconstruction; the three Bayesian recovery methods are ready for focused rerun after the parameterization change. |
