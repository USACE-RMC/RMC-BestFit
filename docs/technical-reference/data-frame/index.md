...
This corrects [issue #19](https://github.com/USACE-RMC/RMC-BestFit/issues/19), introduced between v2-beta.5 and v2.0.0. Sorting previously calculated probabilities is insufficient because the incorrect classification also changes their spacing. The correction retains the current HS recurrence, tie handling, and strict probability bounds. Plotting positions feed empirical distributions and moments, regression-on-order-statistics initialization, and fit diagnostics; changes can therefore affect these derived quantities as well as the frequency plot. Likelihood definitions and observation data are unchanged.
...
The application applies a separate compatibility repair in `InputData.Open`: valid frames with explicit observations below covering perception thresholds have their plotting positions recalculated once before normal change listeners attach. If any position changes, the element is marked dirty so ordinary saving persists the repair. Invalid frames continue through the existing validation path, and unaffected frames retain their supplied positions. Opening does not rerun saved fits; reprocess affected analyses when updated derived statistics or fitted results are required. This application repair does not change the direct XML-construction contract above.
