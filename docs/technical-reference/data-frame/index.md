...
A nonstationary model instead evaluates parameters $\theta(t)$ at each item in `FullTimeSeries`. `CreateFullTimeSeries()` merges explicit records and expands aggregate threshold periods to single-index records. It assigns below-threshold records first and above-threshold records to the last portion of the inclusive period, skipping indexes occupied by explicit observations. This creates a deterministic chronology, but aggregate counts do not themselves identify the years of exceedance. Therefore, using aggregate threshold counts with a nonstationary model adds an ordering assumption. Analysts must establish that this convention is defensible or represent historically dated events explicitly.
