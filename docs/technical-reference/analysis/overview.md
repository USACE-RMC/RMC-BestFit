...
`IAnalysis` does not expose `ClearResults`, and callers cannot set `IsEstimated` through the interface. Individual concrete analyses may expose additional result-reset or post-processing methods.
...
Persist only usable results, and store their terminal state and completeness alongside them.

## Batch execution

`BatchAnalysisRunner` coordinates `IAnalysis` instances through `BatchAnalysisOptions` and per-analysis `BatchAnalysisResult`, with progress, cancellation, and checks for missing estimated prerequisites. Shared data or upstream analyses still induce dependence among outputs.

Record input order, options, dependencies, runtimes, and software versions. Check each result's terminal state.

## Serialization boundary

Concrete analyses own their XML reconstruction rules; neither `IAnalysis` nor `AnalysisBase` declares a universal `ToXElement`. Use the concrete class's constructor and methods. Configuration, estimates, and derived results can have different persistence rules, including recomputation or invalidation after input changes.
