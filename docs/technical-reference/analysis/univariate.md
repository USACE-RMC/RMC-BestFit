...
Let $D$ denote the complete `DataFrame`, $\boldsymbol\theta$ the model parameter vector in the order exposed by `Parameters`, and $M$ the selected family and trend specification. The sampler target is
...
Let retained MCMC draws be $\boldsymbol\theta^{(1)},\ldots,\boldsymbol\theta^{(B)}$. For each requested AEP $\alpha_j$, the analysis evaluates
...
- `PosteriorMode` applies the saved draw with the highest posterior target (`MCMCResults.MAP`).

Neither curve is generally equal to the posterior mean of the quantile because $F^{-1}(p\mid\boldsymbol\theta)$ is nonlinear in $\boldsymbol\theta$:
...
For an indexed covariate or time value $t$, trend models map global coefficients to distribution parameters $\boldsymbol\theta(t)$. The chronology curve is
...
Grouped perception-threshold periods use the deterministic disaggregation assumptions documented in [Data Frame and Mixed-Observation Likelihood](../data-frame/index.md#stationary-and-nonstationary-chronology). In addition, coefficient priors apply to the stored trend coefficients, while distribution-dependent Jeffreys and quantile-prior terms are evaluated once at the last, most-recent observed index as documented in [Parameters, Bounds, and Priors](../models/parameters-and-priors.md#complete-univariate-prior). Consequently, nonstationary posterior and chronology results are conditional on both the grouped-threshold allocation and the final-time prior reference.
