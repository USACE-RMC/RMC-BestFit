...
`Bulletin17CAnalysis.RunAsync` first processes threshold series, installs `LinkController.ForLocationScaleShape()`, initializes parameters, and constructs the deterministic penalty delegate. Initial values and bounds come from the Numerics parent distribution. If low outliers or threshold records exist, `GetNonparametricMomentsROS(useLog10)` supplies a regression-on-order-statistics starting point, which the parent converts to parameters; the ROS estimate needs finite plotting positions, and when it throws or returns non-finite moments the model keeps the constraint-based initial values and records a validation warning (an outright initialization failure keeps the parameter shells and records a validation error) rather than leaving the model without parameters. An out-of-bounds or failed initial value is replaced by the midpoint of its bounds.

Bootstrap refits use a broader deterministic candidate set. For a fixed simulated data frame and one randomized penalty target, the model constructs: censoring-bounded midpoint moments, in which each observed low outlier $x$ is replaced only for initialization by $(x+T)/2$ before any base-10 transform; ROS moments; the Numerics distribution default; and the fitted parent parameters. It removes invalid and duplicate candidates, normalizes them inside the model bounds, scores them with the same identity-weight penalized objective used by the first GMM pass, and tries them in increasing objective order. Candidate changes are optimizer restarts for the same statistical realization. A finite result is usable when the inner optimizer reports `Success` or iterative GMM confirms `ConvergedWithinTolerance`; pivotal refits additionally require a usable covariance. Only after every candidate fails these direct numerical checks does the outer loop simulate a new realization and randomized penalty target.
...
- Midpoint, ROS, distribution-default, and parent initialization candidates assist numerical convergence but are not alternative estimators; the accepted result is always the GMM refit.
