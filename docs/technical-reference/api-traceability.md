...
| `Bulletin17CDistribution`, `Bulletin17CAnalysis`, `UncertaintyMethod`, `CohnConfidenceIntervalResult` | Bulletin 17C | B17C model and analysis | Fast moment/covariance/scope tests, seven published examples, three PeakFQ cells, and the current 1,000-refit reliability diagnostic (947 outer-converged, 53 capped) | Complete for published parameter parity and reliability; numerical Cohn values and broad coverage are outside the current claim set |
...
| `MixtureModel`, `MixtureAnalysis` | Mixture models | Full-$K$ public/EM boundary; identified $K-1$ posterior storage | Fast residual-weight, prior, covariance, diagnostic, persistence, and frequency-curve contracts; an independent scikit-learn optimum and retained identified recovery cells | Complete for the declared mixture matrix |
| `CompetingRisksModel`, `CompetingRiskAnalysis` | Competing risks | Competing-risks model and analysis | Fast seed/matrix/initialization contracts, four analytical rank/CDF cells, three identified MLE designs, and two independent Bayesian designs | Complete for the reported matrix; the Bayesian maximum uses bounded priors with the optional Jeffreys scale multiplier disabled. Correlated Bayesian recovery is outside the supported claim |
| `CompositeAnalysis`, `CompositeType`, `AverageMethod`, `WeightedUnivariateAnalysis`, `CorrelationMatrix` | Composite analysis and model averaging | Composite analysis support | Fast criterion/matrix/index/seed/immutability tests, two posterior-resampling oracles, and ten closed-form/published/Cartesian cells | Complete for the declared composite and predictive matrix |
...
| Public API | Technical treatment | Primary implementation | Verification evidence | Status |
|---|---|---|---|---|
| `BivariateDistribution`, `BivariateAnalysis` | Bivariate distributions and analysis | Bivariate model and analysis | Independent copula optima, seven recovery cells, unit/integration sources, and compiled workflow | Complete; marginals are conditioned on rather than jointly estimated |
...
| `SpatialGEV`, `SpatialGEVAnalysis`, `SpatialGEVUncertaintyMethod`, `SpatialDistanceMetric` | Spatial GEV hierarchy, likelihood, fitting, distance metric, uncertainty methods, and results | Spatial model and analysis | Likelihood, kriging, and geodesic oracles; criteria, cross-validation, prediction, simulation, dispatch, and eight ten-site by 100-row recovery cells | Complete for the tested network sizes and dependence structures |
...
The documentation test checks every exported scientific type against this matrix. Constructors and behavior-affecting members belong to their type's assigned chapter and are reviewed through its API table and compiled examples. Grouped result properties must state their units, nullability, and interpretation.
