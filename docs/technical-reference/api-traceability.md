...
| `Bulletin17CDistribution`, `Bulletin17CAnalysis`, `UncertaintyMethod`, `CohnConfidenceIntervalResult` | Bulletin 17C | B17C model and analysis | B17C examples, calibration, covariance, and coverage tests | Complete |
| `PointProcessModel`, `PointProcessAnalysis` | Point-process models | Point-process model and analysis | Fast exposure, generator, histogram, and prior tests pass; current exact cells are listed in the report | TR-004/TR-005 implemented; seasonal input and annual mixed-data contracts resolved; exact reruns remain |
| `MixtureModel`, `MixtureAnalysis` | Mixture models | Mixture model and analysis | Phase 4 fast simplex, hurdle, mixed-likelihood, and impossible-row tests pass; three exact Numerics/BestFit recovery-parity methods compile | TR-006/TR-007/TR-008 implemented; focused recovery execution pending |
| `CompetingRisksModel`, `CompetingRiskAnalysis` | Competing risks | Competing-risks model and analysis | Competing-risk unit tests | Complete |
| `CompositeAnalysis`, `CompositeType`, `AverageMethod`, `WeightedUnivariateAnalysis` | Composite analysis and model averaging | Composite analysis support | Composite weighting, cancellation, and cascade tests | Complete |
