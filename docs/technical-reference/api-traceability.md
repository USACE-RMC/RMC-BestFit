...
| `Bulletin17CDistribution`, `Bulletin17CAnalysis`, `UncertaintyMethod`, `CohnConfidenceIntervalResult` | Bulletin 17C | B17C model and analysis | Fast midpoint/ranked-initializer/bounds-repair tests and 14 exact reliability cells passed; 13,000 unguarded finite outputs with zero retries or substitutions; no Phase 3 oracle artifact | TR-017 naming and TR-018/TR-019 reliability closed; TR-016/TR-020/TR-021 open |
...
| `MixtureModel`, `MixtureAnalysis` | Mixture models | Mixture model and analysis | Phase 4 fast simplex, hurdle, mixed-likelihood, and impossible-row tests; three guarded Numerics/BestFit parity methods; three guarded Bayesian generation-and-recovery methods | Complete with TR-006/TR-007/TR-008 |
| `CompetingRisksModel`, `CompetingRiskAnalysis` | Competing risks | Competing-risks model and analysis | Fast seed/matrix contracts plus four exact guarded analytical rank/CDF methods | Complete with TR-012 |
| `CompositeAnalysis`, `CompositeType`, `AverageMethod`, `WeightedUnivariateAnalysis`, `CorrelationMatrix` | Composite analysis and model averaging | Composite analysis support | Fast invalid-criterion, exact-zero RMSE, B17C compatibility, matrix ownership/validation/persistence, cancellation, and cascade tests | TR-013/TR-015 complete; TR-014 deferred |
