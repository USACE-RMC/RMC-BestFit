...
| Fast regression gate | Phase 6 Batch 6.4 gate, 22 August 2026 | Core 3,309; UI 579; App 438; API 498; 0 failures; strict Debug XML builds 0 warnings/errors |
...
| Spatial extremes | Phase 6 in progress - Batches 6.3 and 6.4 complete (22 August 2026) | TR-048 (observed-subset copula marginalization), TR-049 (Gaussian-process densities as prior structure), TR-057 (consistent Godambe estimating equations with explicit failure), TR-091 (exact clone), and TR-050 through TR-053 (leave-one-site-out with reduced training models, held-out covariate rows, retained results, explicit fold accounting) corrected and TR-055 closed: the eight `mvtnorm` oracle cells, the guarded row/year criteria cell, the three cross-validation parity/accounting cells, and the nine complete-data recovery cells pass under production defaults; TR-054, TR-056, and TR-058 through TR-062 follow in Batches 6.5-6.6. See [Spatial Extremes Verification](spatial-extremes.md). |
