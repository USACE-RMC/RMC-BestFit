...
| Fast regression gate | Phase 6 Batch 6.5 gate, 22 August 2026 | Core 3,323; UI 579; App 438; API 498; 0 failures; strict Debug XML builds 0 warnings/errors |
...
| Spatial extremes | Phase 6 in progress - Batches 6.3 through 6.5 complete (22 August 2026) | TR-048, TR-049, TR-057, TR-091 (likelihood core and clone), TR-050 through TR-053 (leave-one-site-out), and TR-054, TR-056, TR-058, TR-061, TR-062, TR-092, TR-093 (conditional Gaussian-process prediction, temporal block bootstrap, per-draw regional posterior, dependent simulation, uncertainty-method dispatch, likelihood guard, link-space error bounds) corrected and TR-055 closed: the `mvtnorm` and conditional-GP oracle cells, the criteria, cross-validation, prediction, regional, simulation, and dispatch cells, and the recovery cells pass under production defaults; TR-059 and TR-060 follow in Batch 6.6. See [Spatial Extremes Verification](spatial-extremes.md). |
