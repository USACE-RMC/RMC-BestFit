...
| Fast regression gate | Phase 6 Batch 6.3 gate, 21 August 2026 | Core 3,299; UI 579; App 438; API 498; 0 failures; strict Debug XML builds 0 warnings/errors |
...
| Spatial extremes | Phase 6 in progress - Batch 6.3 complete (21 August 2026) | TR-048 (observed-subset copula marginalization), TR-049 (Gaussian-process densities as prior structure), and TR-057 (consistent Godambe estimating equations with explicit failure) corrected, TR-055 closed, and TR-091 (clone dropped the copula/error parameter blocks; found by the acceptance runs) fixed: the eight `mvtnorm` oracle cells, the guarded row/year criteria cell, and the nine complete-data recovery cells pass under production defaults; TR-050 through TR-054, TR-056, and TR-058 through TR-062 follow in Batches 6.4-6.6. See [Spatial Extremes Verification](spatial-extremes.md). |
