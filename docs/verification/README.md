...
| Rating curve | Phase 6 in progress - Batch 6.1 planned | TR-043 through TR-045 open; recovery cells will replicate the three synthetic cases of `examples/6-rating-curve-analysis` under production defaults. See the [Phase 6 ledger](verification-finalization-plan.md#phase-6---rating-curve-bivariate-and-spatial-models). |
| Bivariate and coincident frequency | Phase 6 in progress - Batch 6.2 planned | TR-047 closed. Copula MPL/IFM parity, Bayesian copula recovery (7/7), and closed-form coincident-frequency cells (3/3) passed on 21 August 2026 under production defaults; the chapter and a provenance artifact are planned. |
| Spatial extremes | Phase 6 in progress - Batches 6.3 through 6.6 planned | TR-048 through TR-062 open (TR-055 closed with caveats); no chapter or oracle artifact exists yet. |
