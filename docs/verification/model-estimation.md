...
Status: TR-026, TR-032, and TR-034 passed their approved corrections. Fixed-weight one-step and efficient two-step sandwich covariance both match the self-checking R oracle. TR-085 (22 August 2026) removed the eigenvalue cap from the moment-covariance conditioning of the sandwich and of the post-estimate weighting matrix: the cap rewrote the moment covariance whenever its eigenvalues spanned more than fifty times their median (real-space three-parameter families), so the Pearson Type III covariance cells of `B17CCovarianceTests` reported 36.2 versus 22.4 (n = 25) and 3.04 versus 3.92 (n = 100) for the variance of the mean; with the positive-definite floor alone the sandwich reproduces the Numerics asymptotic oracle, which an independent influence-function derivation confirms. After the change the four methods below, the gradient and influence cells, the GMM recovery cells, and the twelve penalty cells pass (34/34), and `B17CCovarianceTests` passes 13/13; the fast regression `GeneralizedMethodOfMomentsCovarianceScaleTests` pins the closed-form mean and scale variances.
