...
| 6.3 | Spatial likelihood core: TR-048 observed-site copula submatrices, TR-049 data/prior decomposition with the Gaussian-process layer as prior structure, TR-057 consistent Godambe sensitivity/variability with explicit failure, TR-055 row/year criteria closure | R `mvtnorm` missing-site copula and location-error artifact; scalar/pointwise identity, gradient, and kernel-invariance cells | In progress - confirmation complete 21 August 2026: TR-048/049/057 confirmed by eight exact guarded cells (three convention/invariance cells pass); fix plans await approval |
