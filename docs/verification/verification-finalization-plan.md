...
| 6.1 | Rating curve: TR-043 discharge-space Jacobian, TR-044 strictly positive exponent bound with two-sided continuity, TR-045 aligned-pair validation; recovery cells replicating the three synthetic cases of `examples/6-rating-curve-analysis` under production defaults | Base-10 lognormal density oracle (Numerics `LogNormal` plus SciPy artifact); analytical continuity limits; example-fixture artifact (300-observation shipped data plus a seeded 1,000-observation block) with independent SciPy conditional-MLE optima and standard errors; `RatingCurveExampleRecoveryTests` (3 MLE + 3 Bayesian) | Complete - 21 August 2026: TR-043/044/045 corrected as approved (discharge-space density; exponent lower bound 0.1 with legacy warning; aligned-pair validation with unmatched-record warning); three TR-043 cells, seven continuity cells, six replication cells, and twenty legacy recovery cells pass at 1,000 observations; fast core 3,281+; chapter, register, technical reference, release notes updated |
| 6.2 | Bivariate evidence consolidation (TR-047 closed): chapter, copula-estimation provenance artifact, 21 August default-settings results | Independent Python copula density/MPL/IFM artifact; existing R `copula` targets retained as historical values | Complete - 21 August 2026: `copula-estimation-oracle.json` (12 fixtures, densities self-checked), `CopulaEstimationOracleTests` 12/12 through the guarded runner, the bivariate TR-047 routing regression added to the fast suite, and the `bivariate.md` chapter |
| 6.3 | Spatial likelihood core: TR-048 observed-site copula submatrices, TR-049 data/prior decomposition with the Gaussian-process layer as prior structure, TR-057 consistent Godambe sensitivity/variability with explicit failure, TR-055 row/year criteria closure | R `mvtnorm` missing-site copula and location-error artifact; scalar/pointwise identity, gradient, and kernel-invariance cells; guarded row/year criteria cell | Complete - 21 August 2026: TR-048/049/057 corrected, TR-055 closed, and TR-091 (clone structure, found by the acceptance runs) fixed as approved (additive API: observed-subset `GaussianCopula.LogPDF`, `SpatialGEV.PriorLogLikelihood`, `GodambeCovarianceStatus`/`Diagnostic`); eight oracle cells, the criteria cell, and nine recovery cells pass; fast gates Core 3,299, UI 579, App 438, API 498; chapter, register, technical reference, release notes updated |
| 6.4 | Spatial leave-one-site-out: TR-050 result retention, TR-051 actual reduced training model, TR-052 held-out covariates, TR-053 failed-fold accounting | Reduced-model parity (fast and guarded), covariate-row extraction, NaN-fold aggregation contracts | Complete - 22 August 2026: all four corrected as approved (reduced training model per fold with fold analyses carrying the main settings and seed, no restoration refit; held-out covariate rows and `PredictWithCovariates` throwing without covariates; fold status/messages/counts, NaN metrics, aggregates over successful folds, exception only when none succeed); three guarded cells and the fast contracts pass; fast gates Core 3,309, UI 579, App 438, API 498 |
| 6.5 | Spatial prediction, uncertainty, simulation, and dispatch: TR-054 conditional Gaussian-process prediction per draw, TR-058 per-draw regional statistics, TR-061 dependent simulation, TR-056 replicate bootstrap refit, TR-062 uncertainty-method dispatch with result metadata | R conditional-GP artifact (`spatial-conditional-gp-oracle.json`); per-draw regional recomputation; seeded dependence targets; bootstrap structural contracts; injected-result dispatch contracts | Complete - 22 August 2026: all five corrected as approved, plus TR-092 (likelihood guard) and TR-093 (link-space latent-error bounds) found by the confirmation runs; the kriging oracle cell, six guarded acceptance cells, 15 regression cells, and the fast contracts pass; fast gates Core 3,323, UI 579, App 438, API 498 |
| 6.6 | Spatial configuration: TR-059 heuristic-weight rename with obsolete alias, TR-060 additive distance metric (Cartesian default, validated geodesic option) | Forwarding and identity contracts; geodesic-distance artifact (`geodesic-distance-oracle.json`); Euclidean golden regression | Complete - 22 August 2026: `ComputeCorrelationHeuristicSiteWeights` with the obsolete alias and corrected remarks; `SpatialDistanceMetric` with the range prior unchanged in both metrics; two guarded oracle cells and 9 regression cells pass; fast gates Core 3,328, UI 579, App 438, API 498 |
| Exit | Chapters `rating-curve.md`, `bivariate.md`, and `spatial-extremes.md`; book order; README rows; MANIFEST rows; test inventory; register rows and sections; technical-reference chapters and appendices; API baseline additions; release notes; PROGRESS; plan checkpoint | - | Complete - 22 August 2026: every deliverable was delivered with its batch; the spatial chapter and this plan carry the marker `phase-6-complete`, and the checkpoint paragraph and Fresh-Session Prompt are rewritten |
...
Decisions recorded on 22 August 2026 (Haden Smith): TR-059 name `ComputeCorrelationHeuristicSiteWeights` with an obsolete alias; TR-060 distance metric with the range prior unchanged in both metrics; TR-091 clone fix approved; TR-050/TR-051 approved as proposed (reduced training models with fold analyses, no restoration refit); TR-052 approved with the throw on missing covariates; TR-053 approved with status, NaN metrics, and counts, throwing only when no fold succeeds; TR-092 approved (negative-infinite likelihood for non-finite site parameters); TR-093 approved (link-space spread × 3, floor 1.0); TR-054 approved with the conditional residual sampled by default; TR-058 approved (per-draw regional posterior, endpoint averages removed); TR-061 approved; TR-056 approved (temporal block bootstrap keeping all sites, MAP refit per replicate, 50% minimum); TR-062 approved with Gaussian parameter draws N(MAP, Σ) for the Godambe path.

Decisions recorded on 21 August 2026 (Haden Smith): TR-043 approved as proposed (discharge-space density in all three likelihood paths); TR-044 approved with default exponent lower bound 0.1, legacy bounds restored verbatim plus a validation warning, no clamp; TR-045 approved (aligned-pair error plus a non-blocking unmatched-record warning with counts); the replication acceptance rule approved and then amended after the first runs - curve checks compare the BestFit curve with the independent optimum's curve (0.5% MLE, 2% sampled MAP) and keep the true curve within 10%, the production MLE may sit within 1e-4 of the independent optimum (Differential Evolution convergence margin), single-realization band coverage is reported rather than asserted, Bayesian point comparisons use the sampled MAP from `MCMCResults`, and all recovery cells use 1,000-observation fixtures (the shipped 300-observation example is kept for the likelihood oracle and documented as weakly identifying the third control); TR-048, TR-049, TR-057, and TR-055 approved as proposed.

Oracle environment: the locked R 4.4.3 (`mvtnorm`, `jsonlite`, `digest`, `sandwich`; `evd` and `sf` are not installed, so GEV densities and geodesic distances are implemented in the scripts) and Python (`numpy`, `scipy`) environments; no package additions are planned. Tolerances follow the acceptance section; the declared exceptions are `1e-4` scaled parameters for global optimizers and `1e-4` relative for numerically differentiated Godambe matrices.
...
- Parameter-recovery cells use fixtures of 300 to 1,000 observations and prefer 1,000 (decision of 21 August 2026); coverage and small-sample studies keep their designed sizes. Production estimates are compared with an independent optimum as well as with the generating truth, Bayesian point comparisons use the sampled MAP from `MCMCResults`, asymptotic standard errors are recorded in the artifact, and no seed is searched when a realization lands away from the truth.
