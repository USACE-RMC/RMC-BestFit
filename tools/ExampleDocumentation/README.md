...
- `render_docs.py --only <chapter-or-slug>` checks local links and builds HTML previews under `artifacts/example-documentation/site/`. Install `markdown-it-py` alongside the plotting requirements, then serve the repository root on loopback to inspect these pages. The preview does not replace individual figure review.
...
An optional `legendLocation` selects `best`, `upper left`, `upper right`, `lower left` or `lower right` when automatic placement covers observations. This display-only choice is recorded in the PlotSpec. The reviewed description edits and persisted database hashes are recorded in `examples/metadata-audit.json`; hash the database only after closing the SQLite writer so WAL changes have reached the main file.
