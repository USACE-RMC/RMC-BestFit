...
namespace RMC.BestFit.Analyses
{
    ...
    /// Y(t) = μ + φ1*(Y(t-1) - μ) + ... + φp*(Y(t-p) - μ) + ε(t)
    /// where ε(t) ~ N(0, σ²).
    /// </para>
    /// <para>
    ...
    public class ARAnalysis : AnalysisBase
    {
        ...
        /// <see cref="AutoRegressive.PropertyChanged"/> event and updates
        ...
        public async Task UpdatePointEstimateResultsAsync()
        {
            ...
            await Task.Run(() =>
            {
                ...
                double mapLogLH = AutoRegressive.LogLikelihood(BayesianAnalysis.Results.MAP.Values);
