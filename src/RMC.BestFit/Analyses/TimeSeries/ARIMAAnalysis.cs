...
{
    /// <summary>
    /// Performs Bayesian MCMC estimation for an ARIMA (Autoregressive Moving Average) time series model.
    /// </summary>
    /// <remarks>
    /// <para>
    ///     <b>Authors:</b>
    ...
    /// </para>
    /// </remarks>
    public class ARIMAAnalysis : AnalysisBase
    {
        #region Construction

        /// <summary>
        /// Initializes a new instance of the <see cref="ARIMAAnalysis"/> class
        /// for the specified ARIMA model.
        /// </summary>
        /// <param name="arma">
        /// The <see cref="ARIMA"/> model to be estimated.
        /// </param>
        /// <exception cref="ArgumentNullException">
        /// Thrown if <paramref name="arma"/> is <c>null</c>.
        /// </exception>
        public ARIMAAnalysis(ARIMA arma)
        {
            ARIMA = arma ?? throw new ArgumentNullException(nameof(arma));
            BayesianAnalysis = new BayesianAnalysis(arma);
        }
        ...
        /// </summary>
        /// <param name="arma">
        /// The <see cref="ARIMA"/> model associated with this analysis.
        /// </param>
        /// <param name="xElement">
        /// The XML element from which to restore the analysis configuration and results.
        /// </param>
        /// <exception cref="ArgumentNullException">
        /// Thrown if <paramref name="arma"/> or <paramref name="xElement"/> is <c>null</c>.
        /// </exception>
        public ARIMAAnalysis(ARIMA arma, XElement xElement)
        {
            ...
            {
                BayesianAnalysis = new BayesianAnalysis(arma, bayesElement);
            }
            else
            {
                BayesianAnalysis = new BayesianAnalysis(arma);
        ...
        public ARIMA ARIMA
        {
            ...
            private set
            {
                if (_arma != null)
                {
                    _arma.PropertyChanged -= Model_PropertyChanged;
                }
                ...
                if (_arma != null)
                {
                    if (_bayesianAnalysis != null)
                        _bayesianAnalysis.Model = _arma;

                    _arma.PropertyChanged += Model_PropertyChanged;
                }

                RaisePropertyChange(nameof(ARIMA));
            }
        }
        ...
        /// and posterior sampling for the <see cref="ARIMA"/> model.
        /// </para>
        /// </remarks>
        ...
        /// <summary>
        /// Handles property changes on the <see cref="ARIMA"/> model.
        ...
        {
            if (e.PropertyName == nameof(ARIMA.Parameters) ||
                e.PropertyName == nameof(ARIMA.SetDefaultParameters))
            {
            ...
            else if (e.PropertyName == nameof(ARIMA.TimeSeries) ||
                     e.PropertyName == nameof(ARIMA.POrder) ||
                     e.PropertyName == nameof(ARIMA.DOrder) ||
                     e.PropertyName == nameof(ARIMA.QOrder) ||
                     e.PropertyName == nameof(ARIMA.IncludeIntercept) ||
                     e.PropertyName == nameof(ARIMA.TransformType) ||
                     e.PropertyName == nameof(ARIMA.TrainingTimeSteps) ||
                     e.PropertyName == nameof(ARIMA.UseDefaultTrainingSteps) ||
                     e.PropertyName == nameof(ARIMA.UseJeffreysRuleForScale))
            {
                ClearResults();
            }
        ...
        public async Task UpdatePointEstimateResultsAsync()
        {
            ...
            await Task.Run(() =>
            {
                // ARIMA.Predict() reads parameters from model state, so the worker
                // uses a local clone to avoid mutating the shared instance (which
                // would race with any concurrent UI binding read of ARIMA.Parameters).
                var localModel = (ARIMA)ARIMA.Clone();
                ...
                var trueValues = ARIMA.TimeSeries.ValuesToArray();
            ...
            ARIMA.SetParameterValues(parameters);
        ...
        /// <remarks>
        /// <para>
        /// The XML representation does not include the underlying <see cref="ARIMA"/>
        ...
        {
            var root = new XElement("ARIMAAnalysis",
