...
{
    /// <summary>
    /// Unit tests for <see cref="CompetingRisksAnalysisController"/> status codes and payloads,
    /// using the real service over an in-memory store. Analyses are created and validated but
    /// never run.
    /// </summary>
    [TestClass]
    public class CompetingRisksAnalysisControllerTests
    {
        /// <summary>
        ...
        /// </summary>
        private CompetingRisksAnalysisController _controller = null!;
        ...
        public void Initialize()
        {
            ...
            _controller = new CompetingRisksAnalysisController(NullLogger<CompetingRisksAnalysisController>.Instance, service);
        }
        ...
        private CreateCompetingRisksAnalysisRequest CreateValidRequest()
        {
            var input = _store.AddInputData(TestAnalyses.CreateInputDataResource());
            return new CreateCompetingRisksAnalysisRequest
            {
        ...
        public async Task Create_Returns201_404_400()
        {
            ...
            Assert.AreEqual("competingRisks", body.Analysis!.Kind);
            ...
            var (missingStatus, _) = Unwrap(await _controller.Create(new CreateCompetingRisksAnalysisRequest
            {
        ...
        public async Task Get_List_Validate_Delete_Lifecycle()
        {
            ...
            Assert.AreEqual(404, guardStatus, "A univariate id must 404 on the competing risks routes.");
