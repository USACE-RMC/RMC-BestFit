...
/// <summary>
/// Unit tests for the <see cref="ThresholdData"/> class.
/// Tests historical perception thresholds essential for flood frequency analysis.
/// </summary>
/// <remarks>
/// <para>
/// <see cref="ThresholdData.NumberBelow"/> has an <c>internal</c> setter because it is a
/// derived value managed by <see cref="DataFrame.ProcessThresholdSeries"/>. Tests that
/// need to construct a <see cref="ThresholdData"/> with a specific <c>NumberBelow</c>
/// (for round-trip / clone / validation coverage in isolation) use the
/// <see cref="CreateThresholdDataForTesting"/> helper, which builds an XElement and
...
public class ThresholdDataTests
{
    ...
    /// <summary>
    /// Constructs a <see cref="ThresholdData"/> with specific count fields by building an
    ...
    /// <returns>A <see cref="ThresholdData"/> with the specified fields.</returns>
    private static ThresholdData CreateThresholdDataForTesting(
        ...
        int numberAbove = 0, int numberBelow = 0, double plottingPosition = 0.0)
    {
        ...
        return new ThresholdData(xElement);
    }
    ...
    {
        // Act
        var threshold = new ThresholdData();
    ...
    {
        // Act
        var threshold = new ThresholdData(1900, 1950, 100000.0);
    ...
    public void Test_Constructor_XElement_RestoresAllProperties()
    {
        ...
        // Act
        var restored = new ThresholdData(xElement);
    ...
    {
        var threshold = new ThresholdData(1950, 1950, 100000.0);
    ...
    /// </summary>
    /// <remarks>
    /// NumberBelow defaults to 0 for an isolated ThresholdData; the sum 0 + 6 = 6 is ≤ 51
    /// (the Duration), so the threshold passes all validation rules. In a DataFrame context,
    /// <see cref="DataFrame.ProcessThresholdSeries"/> would populate NumberBelow to its derived
    ...
    {
        var threshold = new ThresholdData(1950, 1900, 100000.0);
    ...
    {
        var threshold = new ThresholdData(1900, 1950, double.NaN);
    ...
    {
        var threshold = new ThresholdData(1900, 1950, double.PositiveInfinity);
    ...
    {
        var threshold = new ThresholdData(-200000, 1950, 100000.0);
    ...
    /// <summary>
    /// Verifies <see cref="ThresholdData.Clone"/> copies every field and yields an independent instance.
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    /// Verifies <see cref="ThresholdData.ToXElement"/> writes every persisted field as an attribute.
    /// </summary>
    [TestMethod]
    ...
    {
        var original = new ThresholdData(1900, 1950, Math.PI * 10000);
    ...
    {
        var original = new ThresholdData(2000, 2020, 50000.0)
        {
            NumberAbove = 0
        };
    ...
    /// </summary>
    /// <remarks>
    /// Source ThresholdData is constructed via the XElement helper because NumberBelow is
    /// a derived field (the user would only set NumberAbove in normal usage; NumberBelow
    /// would be populated by <see cref="DataFrame.ProcessThresholdSeries"/>). The scenario
    ...
    /// </summary>
    /// <remarks>
    /// NumberAbove = 0 is explicit; NumberBelow defaults to 0 for an isolated ThresholdData.
    /// The validation only requires that counts are non-negative and their sum does not
    /// exceed Duration — 0 + 0 ≤ 51 passes. DataFrame would derive NumberBelow = 51 in a
    /// live context.
    /// </remarks>
    [TestMethod]
    ...
    {
        var threshold = new ThresholdData(1900, 1909, 1000.0)
        {
            NumberAbove = 10
        };
    ...
    public void Test_NegativeIndices_Allowed()
    {
        ...
        var threshold = new ThresholdData(-500, -400, 100000.0)
        {
            NumberAbove = 2
        };
    ...
    {
        var threshold = new ThresholdData(1900, 1950, 1e12); // 1 trillion
    ...
    {
        var threshold = new ThresholdData(1900, 1950, 0.001);
    ...
    {
        var threshold = new ThresholdData(1900, 1950, 0.0);
    ...
    public void Test_NegativeValue()
    {
        ...
        var threshold = new ThresholdData(1900, 1950, -50.0);
