...
/// </remarks>
[TestClass]
public class ExactDataHypothesisTests
{
    /// <summary>
    ...
    /// <summary>
    /// Tests Jarque-Bera test for normality based on sample skewness and kurtosis.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// Note: Results may differ slightly from R due to different moment estimation methods (biased vs. unbiased estimators).
    /// </para>
    /// <b> References: </b>
    /// <list type="bullet">
    /// <item><description> 
    /// Komsta, L. (2005). moments: Moments, Cumulants, Skewness, Kurtosis and Related Tests. R package version 0.14.1.
    /// </description></item>
    /// <item><description>
    ...
    public void Test_JarqueBera()
    {
        ...
        var p = df.JarqueBeraTest();
        double true_p = 3.444E-05 / 2;
        Assert.AreEqual(p, true_p, 1E-5);

        var data2 = new double[] { 4, 5, 5, 6, 9, 12, 13, 14, 14, 19, 22, 24, 25 };
        df.ExactSeries = new ExactSeries(data2);
        p = df.JarqueBeraTest();
        Assert.AreEqual(p, 0.592128, 1E-6);
    ...
    /// freedom equal to the number of lags tested.
    /// </remarks>
    [TestMethod]
    public void Test_LjungBox()
    {
        ...
        var p2 = df.LjungBoxTest(30);
        var true_p2 = 0.7548;
        Assert.AreEqual(p2, true_p2, 1E-4);
    }

    /// <summary>
    /// Tests Wald-Wolfowitz runs test for randomness in a sequence.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// of runs (consecutive observations above or below the median). Too few or too many runs suggest
    /// non-randomness, which may indicate trends, cycles, or other patterns in hydrologic data. The test
    /// is distribution-free and uses a normal approximation for the sampling distribution of the runs statistic.
    /// </para>
    /// </remarks>
    [TestMethod]
    ...
    /// <summary>
    /// Tests Mann-Whitney U test for comparing two independent samples.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// for comparing two samples to assess whether their population distributions differ. It tests
    /// whether observations in one sample tend to be larger than observations in the other, making
    /// it useful for testing temporal shifts in hydrologic records without assuming normality.
    /// </para>
    /// </remarks>
    [TestMethod]
    ...
    /// <summary>
    /// Tests Mann-Kendall test for monotonic trend in time series.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// It compares all pairs of observations to determine if later values tend to be higher (upward trend)
    /// or lower (downward trend) than earlier values. The test is widely used in hydrology for detecting
    /// climate change impacts and non-stationarity in streamflow records.
    /// </para>
    /// <b> References: </b>
    ...
    /// <summary>
    /// Tests Gaussian Mixture Model (GMM) based test for unimodality.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// The test compares the Bayesian Information Criterion (BIC) for one-component vs. two-component
    ...
    public void Test_UnimodalityTest()
    {
        ...
        df.ExactSeries = new ExactSeries(bimodalData);
        pval = df.UnimodalityTest();
    ...
    /// <summary>
    /// Tests Multiple Grubbs-Beck Test (MGBT) for identifying low outliers.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// The MGBT is an iterative procedure that identifies potentially influential low floods (PILFs) by
    /// repeatedly applying the Grubbs-Beck test statistic. Low outliers can have disproportionate influence
    /// on the upper tail of the fitted distribution, so their identification is critical for accurate
    /// extreme flood quantile estimation. Identified outliers are typically censored rather than removed
    /// from the analysis.
    /// </para>
    /// <para>
    /// Tests multiple USGS stream gage records with varying numbers of detected low outliers (0-9),
    /// demonstrating robustness across diverse hydrologic conditions.
    /// </para>
    /// </remarks>
    [TestMethod]
    public void Test_MultipleGrubbsBeck()
    {
        ...
        df.ExactSeries = new ExactSeries(CGR);
        df.SetLowOutliersFromMGBT();
        LO = df.NumberOfLowOutliers;
        Assert.AreEqual(LO, CGR_True);

        df.ExactSeries = new ExactSeries(FCK);
        df.SetLowOutliersFromMGBT();
        LO = df.NumberOfLowOutliers;
        Assert.AreEqual(LO, FCK_True);

        df.ExactSeries = new ExactSeries(FOS);
        df.SetLowOutliersFromMGBT();
        LO = df.NumberOfLowOutliers;
        Assert.AreEqual(LO, FOS_True);

        df.ExactSeries = new ExactSeries(GPV);
        df.SetLowOutliersFromMGBT();
        LO = df.NumberOfLowOutliers;
        Assert.AreEqual(LO, GPV_True);

        df.ExactSeries = new ExactSeries(HCK);
        df.SetLowOutliersFromMGBT();
        LO = df.NumberOfLowOutliers;
        Assert.AreEqual(LO, HCK_True);

        df.ExactSeries = new ExactSeries(LOP);
        df.SetLowOutliersFromMGBT();
        LO = df.NumberOfLowOutliers;
    ...
    /// <summary>
    /// Tests threshold-based low outlier identification.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// </para>
    /// <para>
    /// For each dataset, the threshold is set to match the value identified by MGBT in HEC-SSP, then verifies
    /// that the correct number of observations fall below the threshold. This method is simpler than MGBT
    /// but requires domain knowledge to select appropriate threshold values.
    /// </para>
    /// </remarks>
    [TestMethod]
    public void Test_Threshold()
    {
        ...
        df.ExactSeries = new ExactSeries(CGR);
        df.LowOutlierThreshold = 2050.7;
        df.SetLowOutliersFromThreshold();
        LO = df.NumberOfLowOutliers;
        Assert.AreEqual(LO, CGR_True);

        df.ExactSeries = new ExactSeries(FCK);
        df.LowOutlierThreshold = 2991.3;
        df.SetLowOutliersFromThreshold();
        LO = df.NumberOfLowOutliers;
        Assert.AreEqual(LO, FCK_True);

        df.ExactSeries = new ExactSeries(FOS);
        df.LowOutlierThreshold = 9017.7;
        df.SetLowOutliersFromThreshold();
        LO = df.NumberOfLowOutliers;
        Assert.AreEqual(LO, FOS_True);

        df.ExactSeries = new ExactSeries(GPV);
        df.LowOutlierThreshold = 5636.7;
        df.SetLowOutliersFromThreshold();
        LO = df.NumberOfLowOutliers;
        Assert.AreEqual(LO, GPV_True);

        df.ExactSeries = new ExactSeries(HCK);
        df.LowOutlierThreshold = 1570;
        df.SetLowOutliersFromThreshold();
        LO = df.NumberOfLowOutliers;
        Assert.AreEqual(LO, HCK_True);

        df.ExactSeries = new ExactSeries(LOP);
        df.LowOutlierThreshold = 4560;
        df.SetLowOutliersFromThreshold();
        LO = df.NumberOfLowOutliers;
        Assert.AreEqual(LO, LOP_True);

        df.ExactSeries = new ExactSeries(sample);
        df.LowOutlierThreshold = 0;
        df.SetLowOutliersFromThreshold();
        LO = df.NumberOfLowOutliers;
        Assert.AreEqual(LO, Samp_True);
