...
/// <summary>
/// Programmatic unit tests for the <see cref="ARAnalysis"/> wrapper.
/// </summary>
/// <remarks>
/// <para>
...
public class ARAnalysisTests
{
    ...
    /// <summary>
    /// Deterministic 60-observation annual streamflow fixture with AR(1) structure.
    /// Mean ≈ 5000, persistence φ ≈ 0.6, sigma ≈ 600.
    /// </summary>
    private static TimeSeries CreateAnnualStreamflowTimeSeries()
    {
        ...
        {
            double innovation = (rng.NextDouble() * 2 - 1) * sigma;
            double value = mean + phi * (prevValue - mean) + innovation;
    ...
    /// <summary>
    /// Tests that the ARAnalysis constructor properly initializes with a valid AR model.
    /// </summary>
    [TestMethod]
    public void Test_Constructor_ValidModel_InitializesCorrectly()
    {
        ...
        Assert.IsNotNull(analysis.AutoRegressive);
        Assert.IsNotNull(analysis.BayesianAnalysis);
        Assert.AreSame(model, analysis.AutoRegressive);
    }
    ...
    {
        var analysis = new ARAnalysis(null!);
    }
    ...
    public void Test_XmlSerialization_RoundTrip_PreservesConfiguration()
    {
        ...
        var original = new ARAnalysis(model);
        ...
        var restored = new ARAnalysis(model, xElement);
    ...
    public void Test_XmlSerialization_NullXElement_ThrowsArgumentNullException()
    {
        ...
        var analysis = new ARAnalysis(model, null!);
    }
    ...
    public void Test_ToXElement_CreatesValidStructure()
    {
        ...
        Assert.AreEqual("ARAnalysis", xElement.Name.LocalName);
    ...
    {
        var model = new AutoRegressive(); // No time series
        var analysis = new ARAnalysis(model);
    ...
    public void Test_Validate_NegativeForecastingSteps_HandledCorrectly()
    {
        ...
        // Negative values should be clamped or fail validation
        var (isValid, messages) = analysis.Validate();
    ...
    public void Test_PropertyChange_ForecastingTimeSteps_RaisesPropertyChanged()
    {
        ...
        {
            if (e.PropertyName == nameof(ARAnalysis.ForecastingTimeSteps))
    ...
    /// <summary>
    /// Tests that AutoRegressive property returns correct model reference.
    /// </summary>
    [TestMethod]
    public void Test_AutoRegressive_ReturnsCorrectModelReference()
    {
        var ts = CreateAnnualStreamflowTimeSeries();
        var model = new AutoRegressive(ts, order: 2);
        ...
        Assert.AreSame(model, analysis.AutoRegressive);
        Assert.AreEqual(2, analysis.AutoRegressive.Order);
    }
    ...
    /// <summary>
    /// Tests analysis with higher order AR model.
    /// </summary>
    [TestMethod]
    public void Test_EdgeCase_HigherOrderAR()
    {
        var ts = CreateAnnualStreamflowTimeSeries();
        var model = new AutoRegressive(ts, order: 5);
