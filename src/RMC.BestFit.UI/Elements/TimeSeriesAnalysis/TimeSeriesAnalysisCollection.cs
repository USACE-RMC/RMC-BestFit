...
{
    /// <summary>
    /// The time series analysis collection.
    /// </summary>
    /// <remarks>
    /// <para>
    ///     Authors:
    ...
    /// </para>
    /// </remarks>
    public class TimeSeriesAnalysisCollection : ElementCollectionBase
    {
        /// <summary>
        ...
        public TimeSeriesAnalysisCollection(IProject parentProject) : base(parentProject) { }

        /// <summary>
        /// The name of the element collection.
        /// </summary>
        public override string Name => "Time Series Analysis";
        ...
        public override void Save()
        {
            ...
            try
            {
                if (_needsTableCompaction)
                {
                    ...
                    {
                        ((TimeSeriesAnalysis)ElementList[i]).RaisePreviewSaved(ref cancel);
                        if (cancel == true) return;
                    }
        ...
        public override void Open()
        {
            ...
            try
            {
                if (sqlite.TableNames.Contains(Name) == true)
                {
                    ...
                    {
                        var element = new TimeSeriesAnalysis(elementName, this, true);
                        Add(element);
                    }
        ...
        public override void Add(IElement item)
        {
            ...
            ElementList.Add((TimeSeriesAnalysis)item);
            if (_opening == false)
            {
                var sqlite = new SQLiteManager(ParentProject.FullFileName);
                sqlite.Open();
        ...
        public override void InsertFromExternalProject(int index, string elementName, string elementType, string fullFileName)
        {
            ...
            if (CollectionPersistenceHelper.GetClassName(elementType) == nameof(TimeSeriesAnalysis))
            {
                var element = new TimeSeriesAnalysis(elementName, this);
                element.Open(sqlite);
        ...
        public override void Insert(int index, IElement item)
        {
            item.PropertyChanged += ElementPropertyChanged;
            item.Deleted += ElementDeleted;
            ElementList.Insert(index, (TimeSeriesAnalysis)item);
            if (_opening == false)
            {
                var sqlite = new SQLiteManager(ParentProject.FullFileName);
                sqlite.Open();
