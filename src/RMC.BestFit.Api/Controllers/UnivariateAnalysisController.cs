...
{
    /// <summary>
    /// Endpoints for Bayesian MCMC univariate frequency analyses: create against an input-data
    /// resource, run synchronously, and retrieve frequency curves with full posterior uncertainty.
    /// </summary>
    [Route("api/analyses/univariate")]
    public class UnivariateAnalysisController : ApiControllerBase
    {
        /// <summary>
        /// The controller's logger.
        /// </summary>
        private readonly ILogger<UnivariateAnalysisController> _logger;
        ...
        public UnivariateAnalysisController(ILogger<UnivariateAnalysisController> logger, IAnalysisService service)
        {
        ...
        /// <summary>
        /// Creates a univariate frequency analysis linked to an input-data resource. The analysis
        /// clones the input data, so later edits or deletes of the input-data resource cannot
        /// affect it. Creation validates the configuration and reports messages without rejecting.
        /// </summary>
        ...
        /// <response code="400">The distribution, ordinates, or Bayesian options are invalid.</response>
        ...
        public Task<ActionResult<AnalysisResourceResponse>> Create([FromBody] CreateUnivariateAnalysisRequest request)
        {
            return ExecuteAsync(() => AnalysisMapper.ToResourceResponse(_service.CreateUnivariate(request)),
                _logger, "analyses.univariate.create", StatusCodes.Status201Created);
        }
        ...
        /// <response code="404">No univariate analysis has the id.</response>
        ...
        {
            return ExecuteAsync(() => _service.RunFrequencyAsync(id, AnalysisKind.Univariate, cancellationToken),
                _logger, "analyses.univariate.run");
        }

        /// <summary>
        /// Lists all univariate analyses.
        /// </summary>
        /// <returns>Summaries of every univariate analysis.</returns>
        ...
        {
            return ExecuteAsync(() => AnalysisMapper.ToListResponse(_service.List(AnalysisKind.Univariate)),
                _logger, "analyses.univariate.list");
        }
        ...
        {
            return ExecuteAsync(() => AnalysisMapper.ToResourceResponse(_service.Get(id, AnalysisKind.Univariate)),
                _logger, "analyses.univariate.get");
        }
        ...
        /// <response code="404">No univariate analysis has the id, or it has never been run.</response>
        [HttpGet("{id:guid}/results")]
        [ProducesResponseType(typeof(FrequencyResultsResponse), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(FrequencyResultsResponse), StatusCodes.Status404NotFound)]
        public Task<ActionResult<FrequencyResultsResponse>> GetResults(Guid id)
        {
            return ExecuteAsync(() => _service.GetFrequencyResults(id, AnalysisKind.Univariate),
                _logger, "analyses.univariate.results");
        }
        ...
        public ActionResult<ValidationResponse> Validate(Guid id)
        {
            try
            {
                return Ok(_service.Validate(id, AnalysisKind.Univariate));
            }
        ...
        public Task<ActionResult<DeleteResourceResponse>> Delete(Guid id)
        {
            return ExecuteAsync(() =>
            {
                _service.Delete(id, AnalysisKind.Univariate);
                return new DeleteResourceResponse { DeletedId = id, ResourceType = "analysis" };
            }, _logger, "analyses.univariate.delete");
        }
    }
}
